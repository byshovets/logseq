// Self-host MVP smoke test (the CI-worthy end-to-end check).
//
// Verifies, with NO manual login, that: browser A auto-logs-in, creates a graph,
// uploads it, and an edit syncs to the server; then a fresh browser B auto-opens
// that graph and shows the edit. This is the A->B proof the whole MVP rests on.
//
...
(async () => {
  console.log(`smoke: APP=${APP} DATA_DIR=${DATA_DIR}`);
  ...
  await A.evaluate(() => frontend.handler.db_based.sync._LT_rtc_upload_graph_BANG_(frontend.state.get_current_repo(), false).catch(() => {}));
  for (let i = 0; i < 30 && !graphStats().length; i++) await A.waitForTimeout(1000);
  ...
  console.log('A: graph created, uploaded, edit synced');
  await ctxA.close();
